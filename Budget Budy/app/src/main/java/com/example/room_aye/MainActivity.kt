...
import com.example.room_aye.screens.add_new_entry.AddNewTransactionActivity
...
class MainActivity : AppCompatActivity(), Serializable {
        ...
        val startForResult =
            registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result: ActivityResult ->
                ...
                if ((result.resultCode == Activity.RESULT_OK) &&
                    result.data?.getSerializableExtra(AddNewTransactionActivity.ADD_NEW_ENTRY) != null
                ) {
    ...
    private fun saveNewEntry(data: Intent?) {
        data?.getSerializableExtra(AddNewTransactionActivity.ADD_NEW_ENTRY)?.let {
