...
interface TransactionsDao {
    ...
    // All
    @Query("SELECT *, sum(amount) AS catAmount, sum(amount) * 100.0 / (select sum(amount) from transactions_table) as percentage FROM transactions_table GROUP BY category_rowValue ORDER BY sum(amount) DESC")
    fun fetchReportingAll(): Flow<List<TransactionsTable>>

    // Week, Year
    @Query("SELECT *, sum(amount) AS catAmount, sum(amount) * 100.0 / (select sum(amount) from transactions_table where time_range_week=:week and time_range_year =:year) as percentage FROM transactions_table WHERE time_range_week =:week AND time_range_year =:year GROUP BY category_rowValue ORDER BY sum(amount) DESC")
    fun fetchReportingByWeekAndYear(week: Int, year: Int): Flow<List<TransactionsTable>>

    // Quarter, Year
    @Query("SELECT *, sum(amount) AS catAmount, sum(amount) * 100.0 / (select sum(amount) from transactions_table where time_range_quarter=:quarter and time_range_year =:year) as percentage FROM transactions_table WHERE time_range_quarter =:quarter AND time_range_year =:year GROUP BY category_rowValue ORDER BY sum(amount) DESC")
